...
def decrypt_value(encrypted_value):
    ...
    try:
        ...
        logger.warning(
            f"Decryption failed: {type(e).__name__}. This may indicate data corruption or wrong encryption key."
        )
        return None
...
class Matter(models.Model):
    ...
    complainant = models.ForeignKey(
        Person, on_delete=models.SET_NULL, null=True, related_name="complaints"
    )
    ...
    updated_at = models.DateTimeField(auto_now=True)
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="matters_created"
    )
...
class WebhookSubscription(models.Model):
    ...
    secret = models.CharField(
        max_length=255, blank=True, help_text="Optional secret for HMAC signature"
    )
