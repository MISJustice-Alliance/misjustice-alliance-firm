...
class DocumentViewSet(viewsets.ModelViewSet):
    ...
    def upload(self, request):
        ...
        if not matter_id:
            return Response({"error": "matter is required"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if data_tier not in valid_tiers:
            return Response({"error": "Invalid data_tier"}, status=status.HTTP_400_BAD_REQUEST)

        matter_tier_num = int(matter.data_tier.split("-")[1])
        doc_tier_num = int(data_tier.split("-")[1])
        if doc_tier_num > matter_tier_num:
            return Response(
                {"error": "Document tier exceeds matter tier"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        uploaded_path = storage_client.upload_file(
            file_obj, storage_key, mime_type=file_obj.content_type
        )

        if not uploaded_path:
            return Response(
                {"error": "File upload failed"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
