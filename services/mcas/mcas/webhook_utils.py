...
def deliver_webhook(webhook_event):
    ...
    """
    subscriptions = WebhookSubscription.objects.filter(active=True).filter(
...
def send_webhook_request(subscription, webhook_event):
    ...
    if subscription.secret:
        ...
        signature = hmac.new(
            subscription.secret.encode(), payload_json.encode(), hashlib.sha256
        ).hexdigest()
    ...
    response = requests.post(subscription.url, json=payload, headers=headers, timeout=10)
